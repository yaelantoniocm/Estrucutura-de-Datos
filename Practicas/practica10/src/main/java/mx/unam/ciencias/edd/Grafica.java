...
public class Grafica<T> implements Coleccion<T> {
    ...
    @Override public int getElementos() {
        return vertices.getLongitud(); //getElementos
    }
    ...
    private Vecino buscaVecino(Vertice vertice1, Vertice vertice2) {
        for(Vecino vecino: vertice1.vecinos) {
            if(vecino.vecino.equals(vertice2)){ 
                return vecino; 
            }
        }
        return null;
    ...
    public void desconecta(T a, T b) {
        ...
        for (Vecino ve : verticeA.vecinos) {
            if (ve.vecino.equals(verticeB)) {
                ve_ab = ve;
            }
        }
        for (Vecino ve : verticeB.vecinos) {
            if (ve.vecino.equals(verticeA)) {
                ve_ba = ve;
            }
        }
        verticeA.vecinos.elimina(ve_ab);
        verticeB.vecinos.elimina(ve_ba);
        aristas--;
    }
    ...
    public Lista<VerticeGrafica<T>> dijkstra(T origen, T destino) {
        ...
        montMin = new MonticuloMinimo<Vertice>(vertices);
        double aux;
