...
public class ArbolRojinegro<T extends Comparable<T>> extends ArbolBinarioOrdenado<T> {
    ...
    /*Método que se engargara de buscar recursivamente un elemento, me base en la
    ...
    */
    private Vertice busca(Vertice vertice, T elemento) { 
        ...
        if (vertice.get().equals(elemento))
            return vertice;
        Vertice izq = busca(vertice.izquierdo, elemento);
        Vertice der = busca(vertice.derecho, elemento);
